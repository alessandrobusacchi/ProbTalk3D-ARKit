...
def render_video(cfg: DictConfig, wav_path: Path, filename: str) -> None:
    ...
    if os.path.isfile(wav_path):
        cmd = 'ffmpeg -r 25 -i "{}" -i "{}" -pix_fmt yuv420p -s 1280x720 "{}" -y -hide_banner -loglevel error'.format(
            image_temp, wav_path, output_path
        )
    else:
        cmd = 'ffmpeg -r 25 -i "{}" -pix_fmt yuv420p -s 1280x720 "{}" -y -hide_banner -loglevel error'.format(
            image_temp, output_path
        )
